...
export default function SchedulePicker({ nodeLabel, onValueChange }: SchedulePickerProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input type="time" value={time} onChange={e => setTime(e.target.value)} className="text-[6px] text-gray-400"/>
      <input type="date" value={date} onChange={e => setDate(e.target.value)} className="text-[6px] text-gray-400"/>
    </div>
  );
}
