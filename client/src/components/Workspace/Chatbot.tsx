...
const Chatbot: React.FC = () => {
  ...
  const fetchBotResponse = async (userMessage: string) => {
    const message = axios
      .post(`${import.meta.env.VITE_BACKEND_URL}/chatbot/chat`, {
        model: 'llama3.2:1b',
        text: userMessage,
      })
      .then((res) => {
      ...
      <div className="w-full max-w-4xl h-[90vh] flex flex-col">
        <div className="rounded-b-3xl relative">
          ...
          <div className="flex items-center gap-2 rounded-2xl border bg-[#161D2D] border-[#718FD6] shadow-[0px_20px_20px_#2B3340]">
            ...
            >
              <Plus
                className={`text-[#65728F] group-hover:text-white transition-all ${dropdown ? 'rotate-45' : ''}`}
              />
            </button>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-4 pt-6 scrollbar-thumb-emerald-500 scrollbar-thin scrollbar-track-transparent scrollbar-rounded">
          {messages.map((message) => (
            ...
            >
              <div
                className={`rounded-full flex items-center text-white justify-center flex-shrink-0 ${
                  message.type === 'bot'
                    ? ''
              ...
              <div
                className={`max-w-[70%] ${message.type === 'user' ? 'items-end' : 'items-start'} flex flex-col gap-1`}
              >
                <div
            ...
            <div className="flex gap-3">
              <div className="border border-[#718FD6]/30 px-4 py-3 rounded-2xl">
                <div className="flex gap-1">
                  <div
                    className="w-2 h-2 bg-[#65728F] rounded-full animate-bounce"
